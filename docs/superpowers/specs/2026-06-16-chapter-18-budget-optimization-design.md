...
3. **Scenario table (WE2):** re-optimize under $\pm20\%$, channel cap, channel-dark; print
   first-order vs actual $\Delta V$; assert the analytic anchors above.
...
- **B (By hand):** On $r_i=a_i\sqrt b$, $a=(2,1)$: derive $V^\star(B)=\sqrt{5B}$, confirm
  $dV^\star/dB=\lambda^\star$, and compute the budget-$+20\%$ first-order vs exact $\Delta V$.
