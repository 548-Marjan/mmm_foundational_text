...
The single cell below runs the entire chapter on the eight-schools / geo-MMM model with nothing but NumPy and Matplotlib — no PyMC, Stan, or ArviZ — so that every diagnostic is visible as the few lines of arithmetic it actually is. It fits the model twice: once in the **centered** parameterization $(\mu, \log\tau, \theta_1..\theta_J)$ with $\theta_g \sim \mathcal{N}(\mu, \tau^2)$, and once in the **non-centered** parameterization $\theta_g = \mu + \tau\tilde\theta_g$ with $\tilde\theta_g \sim \mathcal{N}(0,1)$, both under a half-normal prior $\tau \sim \text{HalfNormal}(5)$ carried on the $\log\tau$ scale with its Jacobian. A from-scratch leapfrog HMC sampler — lifted directly from Chapter 9 — drives four chains of each, and the diagnostics of Part A and Part B are then computed by hand from the raw draws: split-$\hat{R}$ and effective sample size from the autocorrelation sum, a divergence count flagged whenever the leapfrog energy error $|\Delta H|$ exceeds $1000$ or goes non-finite, a posterior predictive check on the test quantity $T = \max_g y_g$, and a simulation-based-calibration loop run on a cheap conjugate model so the rank histogram is exact and fast. The single experiment is the funnel reckoning Chapter 9 set up: the same $(\varepsilon, L)$ that the centered geometry cannot survive, the non-centered geometry handles cleanly.

```{python}
# Chapter 10 — Code Tie-in: convergence + calibration diagnostics, from scratch.
...
- **Simulation-based calibration (SBC)** is the keystone: under exact inference the rank of a prior-drawn truth among posterior draws is uniform on $\{0, \dots, L\}$. A $\cup$-shaped rank histogram means overconfidence, $\cap$ means under-confidence, a slope means bias. SBC automatically catches the Chapter 9 funnel.
...
**Key identities.** Between- and within-chain variance $B = \frac{N}{M-1}\sum_m(\bar\theta_m - \bar\theta)^2$ and $W = \frac{1}{M}\sum_m s_m^2$; marginal variance estimate $\widehat{\operatorname{var}}^+ = \frac{N-1}{N}W + \frac{1}{N}B$; convergence statistic $\hat{R} = \sqrt{\widehat{\operatorname{var}}^+/W}$; variance inflation $\operatorname{var}(\bar\theta) = \frac{\sigma^2}{N}(1 + 2\sum_k \rho_k)$ giving $N_{\text{eff}} = N/(1 + 2\sum_k \rho_k)$ and $\text{MCSE} = \sigma/\sqrt{N_{\text{eff}}}$; posterior predictive p-value $p_B = \Pr(T(y^{\text{rep}}, \theta) \ge T(y, \theta) \mid y)$; the SBC identity $\int p(\theta\mid y)\,p(y)\,dy = p(\theta)$ making the rank uniform on $\{0, \dots, L\}$; and $\widehat{\text{elpd}}_{\text{WAIC}} = \text{lppd} - p_{\text{WAIC}}$ with $p_{\text{WAIC}} = \sum_i \operatorname{var}_s(\log p(y_i\mid\theta^{(s)}))$. A checked, trustworthy posterior is exactly the input the optimization of Part IV assumes.
